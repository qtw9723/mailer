// supabase/functions/mailer/index.ts
  import { listJobs, createJob, updateJob, deleteJob, 
  getJob, getDueJobs, markSent } from "./db.ts"       
      ...
      const results = await Promise.allSettled(
        jobs.map(async (job) => {
          ...
          for (const recipient of job.recipients) {
            await sendMail({ sender: job.sender, to: recipient,
  subject, body: job.body, attachments:
  job.attachments })
